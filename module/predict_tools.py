...
def predict_llm(InputText, IdRequest, NameBot, User, log_obj):
    ...
    tools = [
        Tool(
            name="Reviews",
            func=review_product,
            description="""trả lời tất cả các câu hỏi được hỏi, trừ hỏi thông tin liên quan đến giá.
            Nếu không tìm được tool  phù hợp thì mặc định sẽ vào đây.
            Pass the entire question as input to the tool.
            For instance, if the question is "Bên mình bán những sản phẩm nào?",
            the input should be "Bên mình bán những sản phẩm nào?"
            """,
        ),
        Tool(
            name="Buy",
            func=take_product,
            description="""Used when asked which products to buy given a specific price.
            Pass the entire question as input to the tool.
            For instance, if the question is "Tôi muốn mua tủ lạnh với giá 20 triệu",
            the input should be "Tôi muốn mua tủ lạnh với giá 20 triệu"
            """,
        ),
    ]
