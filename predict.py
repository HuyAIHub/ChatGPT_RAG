...
def predict_llm(InputText, IdRequest, NameBot, User, log_obj):
    ...
    with Path(path_messages + "/messages_conv.json").open("w") as f:
        json.dump(messages_conv, f, indent=4)
    with Path(path_messages + "/messages_snippets.json").open("w") as f:
        json.dump(messages_snippets, f, indent=4)
