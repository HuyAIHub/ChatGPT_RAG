...
def download_and_index_pdf(urls: list[str]) -> FAISS:
    """
    Download and index a list of PDF, TXT, and DOC documents based on the URLs
    """
    ...
    for url in urls:
        if url.endswith('.pdf'):
            ...
            with open(url, 'r', encoding='utf-8') as file:
                ...
                text = loader.load()
                # Tạo đối tượng Document từ văn bản và nhúng nó
                all_pages.append(Document(text))
        elif url.endswith('.docx'):
            # Đọc văn bản từ tệp tin DOCX và nhúng nó
            doc = Document(url)
